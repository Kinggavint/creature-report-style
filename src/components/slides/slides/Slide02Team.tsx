...
const team = [
  ...
  { name: "Gavin Turnmire", role: "Digital Infrastructure", img: null, desc: "Responsible for Google Ads, GA4 + Tag Manager, SEO, Google Business Profile, and AI tooling." },
      ...
      <div className="grid grid-cols-5 gap-8 flex-1">
        {team.map((m) => (
          <div key={m.name} className="flex flex-col items-center text-center">
            <div className="w-[140px] h-[140px] rounded-full overflow-hidden mb-5 border-2" style={{ borderColor: "#00A896" }}>
              {m.img ? (
                <img src={m.img} alt={m.name} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-[40px] font-bold" style={{ background: "rgba(0,168,150,0.15)", color: "#00A896" }}>
                  {m.name.split(" ").map(n => n[0]).join("")}
                </div>
              )}
            </div>
            <h3 className="text-[24px] font-semibold mb-1" style={{ color: "#FAFAF0" }}>{m.name}</h3>
            <p className="text-[16px] font-medium mb-3" style={{ color: "#D4A853" }}>{m.role}</p>
