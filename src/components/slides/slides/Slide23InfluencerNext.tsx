    ...
    <div className="flex h-full px-16 py-14 gap-10">
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex gap-8 flex-1">
          <div className="flex-1 rounded-xl p-6" style={{ background: "rgba(0,168,150,0.04)", border: "1px solid rgba(0,168,150,0.15)" }}>
            <h3 className="text-[24px] font-semibold mb-6" style={{ color: "#00A896" }}>What Worked</h3>
            <div className="space-y-5">
              {worked.map((w, i) => (
                <div key={i} className="flex items-center gap-4">
                  <CheckCircle size={24} style={{ color: "#00A896" }} />
                  <p className="text-[20px]" style={{ color: "rgba(255,255,255,0.8)" }}>{w}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="flex-1 rounded-xl p-6" style={{ background: "rgba(212,168,83,0.04)", border: "1px solid rgba(212,168,83,0.15)" }}>
            <h3 className="text-[24px] font-semibold mb-6" style={{ color: "#D4A853" }}>Next Steps</h3>
            <div className="space-y-5">
              {nextSteps.map((s, i) => (
                <div key={i} className="flex items-center gap-4">
                  <ArrowRight size={24} style={{ color: "#D4A853" }} />
                  <p className="text-[20px]" style={{ color: "rgba(255,255,255,0.8)" }}>{s}</p>
                </div>
              ))}
            </div>
          </div>
